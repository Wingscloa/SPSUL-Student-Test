...
{
    /// <summary>
    /// Služba pro správu oprávnění učitelů na základě jejich rolí.
    ///
    /// Jak to funguje:
    ///   1. Při každém requestu (přes LoginRequiredAttribute) se načtou role učitele z DB.
    ///   2. Role jsou mapovány na sadu oprávnění (AppPermissions) pomocí statického slovníku.
    ///   3. Výsledná oprávnění jsou UNION všech rolí (pokud má učitel víc rolí).
    ///   4. Oprávnění jsou cachována v IMemoryCache na 10 minut.
    ///
    /// Použití v controlleru:
    ///   bool muze = await _authService.HasPermissionAsync(AppPermissions.CrudTests);
    ///
    /// Použití v Razor view (přes HttpContext.Items, přednačteno LoginRequired):
