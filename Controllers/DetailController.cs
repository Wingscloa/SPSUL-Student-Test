...
{
    /// <summary>
    /// Zobrazování výsledků testů a statistik.
    ///
    /// Funkce:
    ///   Index   – tabulka všech dokončených StudentTest záznamů s filtrováním a stránkováním
    ///   View    – detail jednoho konkrétního výsledku testu (odpovědi studenta vs. správné)
    ///   Stats   – agregované statistiky (průměry, rozdělení úspěšnosti)
    ///   History – historie otázek/testů konkrétního studenta
    ///
    /// Jak se vypočítává úspěšnost:
    ///   Ze StudentTest.ResultSnapshot (JSON) se extrahují odpovědi studenta.
    ///   Porovnají se se správnými možnostmi z Test.QuestionSnapshot (JSON).
    ///   Výsledek je procento správných odpovědí.
    /// </summary>
    [LoginRequired]
